...
export function detectConversationTimeZone(): string {
  ...
  return isValidConversationTimeZone(detected)
    ? detected
    : FALLBACK_CONVERSATION_TIME_ZONE;
}

export function listConversationTimeZones(selectedTimeZone?: string | null): string[] {
  ...
  return Array.from(
    new Set([
      ...
      ...(isValidConversationTimeZone(selectedTimeZone)
...
export function formatConversationTimeZone(
  timeZone: string,
  now = new Date(),
): string {
